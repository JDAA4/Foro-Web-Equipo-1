<?php
    ...
    {
        header("Location:../scr/views/main.html");
    }
?>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    if (isset($_POST['email']) && isset($_POST['password'])) 
    {
        ...
        try {
            ...
            $consulta = $pdo->prepare("SELECT firstname , lastname, password FROM users WHERE email = :email");
            ...
            if ($user) 
            {
                ...
                if (password_verify($password, $user['password']))
                {
                    ...
                    $_SESSION['lastname'] = $user['lastname']; // Almacenamos el apellido del usuario en la sesión
                    header("Location:../src/views/main.html"); // Redireccionar al usuario a la página de inicio
                    exit();
                } else 
                {
